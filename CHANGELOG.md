...
- **A multi-byte UTF-8 character split across two upstream chunks no longer reaches the client as `U+FFFD`.** `streamResponse()` decoded each chunk on its own with `chunk.toString()`, so a CJK character, emoji or full-width punctuation mark that straddled a chunk boundary came out as two or three replacement characters inside the SSE JSON — in the reply, in tool-call arguments, and permanently in the session transcript. The decode is now stateful (`StringDecoder`), with the trailing partial flushed at end-of-stream. ASCII-only traffic was never affected, which is why this went unnoticed; CJK sessions hit it roughly once per few KB of streamed text. (#365)
