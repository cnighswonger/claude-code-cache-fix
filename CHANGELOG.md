...
- **`output-guard` — a last-line invariant on the response path ([#278](https://github.com/cnighswonger/claude-code-cache-fix/pull/278); contributed by [@Gunther-Schulz](https://github.com/Gunther-Schulz)).** A pipeline of body-mutating extensions needs one check that runs last and asks a different question: is what we are about to send still the response the upstream sent, in every way the client depends on? The guard validates structural invariants (`tool_use`/`tool_result` adjacency among them) on the outbound response, and on violation **restores** the original rather than passing the mutation through — a mutation bug becomes one logged line instead of a corrupted session. **This ships as two extension files and is one feature:** `output-guard-stash` at order 55 stashes the pre-mutation body, `output-guard` at order 690 validates and restores from that stash. The pipeline loads one default export per file and the two halves must run at opposite ends of it; the split is scoping, not two features. Its first 243 live firings were its own missing exemption, which is the argument for it — a guard that can restore makes that class of mistake cheap to discover and free to survive.
