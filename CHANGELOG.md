...
- **`model-id-sanitize` extension ([#227](https://github.com/cnighswonger/claude-code-cache-fix/issues/227), refs upstream [anthropics/claude-code#68285](https://github.com/anthropics/claude-code/issues/68285), [anthropics/claude-code#68279](https://github.com/anthropics/claude-code/issues/68279)).** Detects (and optionally rewrites or blocks) malformed outbound `body.model` values caused by CC's `/model` picker writing ANSI SGR bold escape sequences into `~/.claude/settings.json`. Anthropic's API prefix-matches the substring (`opus`, `fable-5`) and resolves it to the current premium variant in that family — a deterministic ~$998 billing event on 2026-06-11 per CC#68285. **Four-mode ladder** via `CACHE_FIX_MODEL_ID_SANITIZE` env var (v1 default: `off`): `off` (extension early-returns; pipeline unchanged), `warn` (detect + log + per-session JSON spread; forward unchanged), `strip` (detect + recover; mutate `body.model`), `block` (detect + reject with synthetic 400 via the existing `pre.handled` short-circuit). **Strip-mode recovery** follows strict precedence: (1) exact canonical full-ID recovery — the load-bearing path that preserves operator intent on the known bad shapes (`claude-fable-5\x1b[1m` → `claude-fable-5`, `claude-opus-4-8\x1b[1m` → `claude-opus-4-8`); (2) family-root fallback to the **oldest-in-family wire ID** when no full canonical is recoverable; (3) no-confidence fallthrough to block behavior when neither exact nor single-family-root recovery succeeds. **Selection rationale for fallback is "oldest in-family", NOT cheapest**: Anthropic's current pricing puts Opus 4.6/4.7/4.8 at the same `$5/$25` MTok point, so picking 4.6 is a capability downgrade at the same price, not a cost saving. Oldest in-family minimizes accidental upgrade from operator intent — if the operator's `settings.json` was corrupted, they almost certainly intended what they had been using, which was the variant available before the latest in-family release. Fable / Mythos targets are an **availability** fallback (both currently suspended per Anthropic's 2026-06-12 notice). **Operator attestation for `strip` mode**: flipping the env-var IS the operator's attestation of preference for "wrong-but-bounded" over "wrong-and-most-expensive"; the family-fallback path can choose a different model than the operator intended when exact-canonical recovery fails. **Block mode** uses a `400 model_not_found` JSON envelope for `stream: false` and an SSE `event: error` frame for `stream: true` — the impl PR carries `needs-sim-validation` for both paths since the existing synthetic-response precedent (`image-retry-circuit-breaker.mjs:249-268`) synthesizes 200s, not 400s. New persisted fields on the per-session JSON (spread idiom mirrors `_thinkingSanitize` / `_auto1mGuard` / `_modelDivergence`): `model_id_malformed` (bool), `model_id_malformed_first_seen` (ISO ts), `model_id_corrections_count` (int — incremented in `strip` mode), `model_id_malformed_last_value_hex` (hex-escaped representation; never raw control bytes per terminal-safety hygiene). Master switch: `CACHE_FIX_MODEL_ID_SANITIZE=off` (the default) disables the extension entirely. Extension registered at order **50** (between `bootstrap-defense:45` and `ttl-tier-detect:75`) so detection / strip runs before any other extension reads `body.model`. **Composition**: introduces `proxy/model-families.mjs` as the single source of truth for the family roster — both this extension and `cache-telemetry.mjs`'s served-model divergence detector (PR #225) import from it, eliminating drift when Anthropic ships new models. See [`docs/directives/proxy-model-id-sanitize.md`](docs/directives/proxy-model-id-sanitize.md) for the full design.
