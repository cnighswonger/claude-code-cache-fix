...
- **`messages-cache-breakpoint` (order 410) and its two env vars, `CACHE_FIX_INJECT_MESSAGES_BREAKPOINT` and `CACHE_FIX_DUMP_MESSAGES_HEAD`.** The extension pinned breakpoint #3 by *position* against head mutation ([anthropics/claude-code#47098](https://github.com/anthropics/claude-code/issues/47098)). `insertion-normalization` recognises a message across re-serializations by content identity instead of pinning its position, which covers the same rationale without spending the 4th `cache_control` slot — the API allows four per request and Claude Code occupies three, so this extension took the last one. Holding that slot is not free even when the extension behaves: any future consumer of the 4th breakpoint silently starves. Injection was opt-in and default-off, so operators who never set `CACHE_FIX_INJECT_MESSAGES_BREAKPOINT=1` see no behavioural change; those who did lose the breakpoint-#3 injection and regain the free slot. Measured after removal on two full-session corpora, replaying the real pipeline: 0 stability, 0 safety, 0 sequence violations. The design record stays at `docs/directives/proxy-messages-cache-breakpoint.md`.
