...
| `CACHE_FIX_FORWARD_ERROR_LOG` | `on` | 프록시가 업스트림 연결 실패로 클라이언트에 502를 반환할 때마다 `[cache-fix] upstream error -> 502: ...` stderr 한 줄을 기록합니다 (오류, 메서드, 경로; 세션 ID는 마스킹됨). 비활성화하려면 `off`로 설정합니다. |
