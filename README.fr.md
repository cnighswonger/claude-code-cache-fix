...
| `CACHE_FIX_FORWARD_ERROR_LOG` | `on` | Journalise une ligne stderr `[cache-fix] upstream error -> 502: ...` (erreur, méthode, route ; identifiants de session masqués) chaque fois que le proxy renvoie un 502 au client suite à un échec de connexion en amont. Mettre à `off` pour désactiver. |
