...
| [#43657](https://github.com/anthropics/claude-code/issues/43657) | Resume/continue cache invalidation | **Closed (stale-bot 2026-06-11) — refiled as [#67497](https://github.com/anthropics/claude-code/issues/67497)** | Was closed, simpolism claimed "fixed in 2.1.97" — we posted test data showing scatter still present. Reopened after our comment (2026-04-09). **Re-closed 2026-06-11T11:42 by github-actions[bot] as `not_planned` for inactivity, NOT resolved.** Posted stale-closure note 2026-06-11 ([comment](https://github.com/anthropics/claude-code/issues/43657#issuecomment-4680702966)) + back-link to refile ([comment](https://github.com/anthropics/claude-code/issues/43657#issuecomment-4680711901)). |
| [#67497](https://github.com/anthropics/claude-code/issues/67497) | v2.1.148: --resume cache invalidation — system-block scatter still present (refile of #43657) | Open | Filed 2026-06-11 as cnighswonger to preserve the regression class against future stale-bot cycles. Documents current binary evidence: proxy `fresh-session-sort` extension at order 250 mitigates by relocating scattered system-blocks to deterministic positions in `messages[0]`. Without extension: `cache_read` drops to near zero on first turn after resume. With: 96-99% hit rate restored. CC-side fix would touch `processSessionStartHooks` + `reorderAttachmentsForAPI` + `normalizeMessagesForAPI`. |
