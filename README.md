...
| `model-id-sanitize` | Detects (and optionally rewrites or blocks) malformed `body.model` values caused by CC's `/model` picker writing ANSI escape sequences into `~/.claude/settings.json` ([CC#68285](https://github.com/anthropics/claude-code/issues/68285), [CC#68279](https://github.com/anthropics/claude-code/issues/68279)). Four modes via `CACHE_FIX_MODEL_ID_SANITIZE`: `off` (default — pipeline unchanged), `warn` (log + per-session JSON spread), `strip` (rewrite to recovered canonical ID or oldest-in-family fallback target), `block` (synthetic 400). |
