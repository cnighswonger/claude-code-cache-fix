...
1. The directive still contains stale dump-shape and dump-timing text that contradicts the revised contract. The pipeline sketch still writes `matched_sentinels` instead of separate `exact_matches` / `partial_matches` ([docs/directives/proxy-microcompact-cache-stability.md](/home/manager/git_repos/claude-code-cache-fix/docs/directives/proxy-microcompact-cache-stability.md:269)), Test 11 still expects `matched_sentinels` ([docs/directives/proxy-microcompact-cache-stability.md](/home/manager/git_repos/claude-code-cache-fix/docs/directives/proxy-microcompact-cache-stability.md:317)), and Test 19 still says the diagnostic captures post-normalization state ([docs/directives/proxy-microcompact-cache-stability.md](/home/manager/git_repos/claude-code-cache-fix/docs/directives/proxy-microcompact-cache-stability.md:329)). Those are not cosmetic leftovers; they reintroduce the same ambiguity the earlier review blocked on.
2. The diagnostic schema example still advertises the old permissive timestamp matcher `^\\[Old tool result content cleared at .+?\\]$` instead of the constrained ISO-8601 form promised by the detection contract ([docs/directives/proxy-microcompact-cache-stability.md](/home/manager/git_repos/claude-code-cache-fix/docs/directives/proxy-microcompact-cache-stability.md:143)). As written, the spec still presents two different accepted timestamp forms to implementers.
