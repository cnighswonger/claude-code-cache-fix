...
- **Preserve `tools[]` array order.** Reordering tools in the request changes which slot which tool occupies in the API's view, which could affect signing. The hash MUST reflect array order. (Note: `sort-stabilization` at order 200 sorts `body.tools` alphabetically by `name` before v2 sees it, so the array-order rule is currently forward-compatibility — defends against any future change in upstream ordering at trivial cost. The recursive key-sort rule on tool-object internals is NOT redundant: no upstream extension normalizes the internal field ordering of tool-definition objects in `body.tools`, so the helper's deterministic recursive stringify is doing real work to handle arbitrary upstream input shapes — `tool-input-normalize` at order 280 normalizes `tool_use.input` keys inside assistant `body.messages[*]` content, not tool definitions in `body.tools`. Different surface.)
