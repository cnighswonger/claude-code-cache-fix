...
- **Failure mode:** if the derivation produces no markers (no-Workflow-traffic case), no derived ids are populated and behavior matches current state. If the operator has set `CACHE_FIX_USAGE_LOG_AGENT_ID=on` without upgrading meter to v0.8.0+, their rows will carry the new fields and fail meter validation — that's their attestation contract, not a runtime guarantee (see Meter compatibility section).
- **Load-bearing? Yes.** Cross-repo wire/schema contract addition: emits two new optional fields on `usage.jsonl` rows whose downstream consumer is the meter at `cnighswonger/claude-code-meter#30` (companion directive). Per CLAUDE.md, schema contract changes require Chris human review before merge in addition to the routine Lead + Codex review path. The schema-change label is applied accordingly.
...
**Operator attestation, not runtime version probe** (closes Fable N5 / Codex round-1 precision + Codex round-2 contradiction): the cache-fix proxy cannot mechanically detect the operator's installed meter version. The env-var `CACHE_FIX_USAGE_LOG_AGENT_ID=on` is the operator's attestation that meter v0.8.0+ is installed. There is no runtime guarantee.

The round-2 directive's wording — "operators on older meter installs see no row changes regardless of the env-var" — was **wrong**, and Codex round-2 correctly caught it as a contradiction with the no-runtime-probe paragraph. The honest contract: if an operator sets `CACHE_FIX_USAGE_LOG_AGENT_ID=on` while running an older meter (v0.7.x or earlier), the emitted row WILL carry `agent_id` / `agent_id_source` keys, and the older meter's strict-object validation WILL reject the row. **That's the operator's attestation failure, not a runtime safety guarantee.** The CHANGELOG must call this out so operators understand the contract: env-var ⇒ "I have meter v0.8.0+ installed"; setting it without upgrading meter breaks their ingestion.

This is exactly the same contract `request_id` shipped under (`CACHE_FIX_USAGE_LOG_REQID=on` is the operator's attestation of meter v0.7.0+; setting it on an older meter would similarly break ingestion). The round-2 directive's "regardless of the env-var" wording implied a runtime probe that does not exist; corrected.
