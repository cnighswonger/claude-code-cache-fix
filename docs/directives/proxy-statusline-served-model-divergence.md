...
**Stage:** directive — round 2 (Fable r1 REQUEST_CHANGES at `9e0d58a`; r2 addresses B1 sticky-state authority + rehydration, B2 pair-keyed counter state, A1 detection moved to `message_delta` inside try/catch, A2 reads from `ctx.telemetry.requestedModel` instead of new plumbing, plus 6 nits)
...
- **Counter reset semantics** (closes Fable r1 B2):
  ...
  - A `/model` invocation that changes the `requestedModel` for subsequent turns starts a fresh pair entry; prior entry's state is retained (so a return to the original `requestedModel` resumes from where it left off) but inactive.
- **Sticky never auto-unlatches within a session — and the map is authoritative across proxy restarts** (closes Fable r1 B1). The per-session JSON is the rehydration source: on map-miss (first turn after proxy restart, fresh process), the writer reads `sessionFilePath(rawSid).json` (precedent: `sessionFilePath` is exported at `cache-telemetry.mjs:55-57` for exactly this read pattern) and rebuilds the entry from any persisted `model_divergence_sticky` / `model_divergence_first_seen` fields. A persisted-sticky entry rehydrates with `sticky: true`; the next turn's writer sees sticky and emits the sticky spread again, so the indicator survives restart. **Clearing sticky requires a new session** — deleting the per-session JSON file clears it AND restarting the proxy (or waiting for the LRU eviction sweep to drop the in-memory entry) clears the map; otherwise the map will re-emit sticky on the next turn. The directive states this explicitly so operators don't expect file-deletion alone to work.
...
4. **Module-scope state — pair-keyed, rehydrated from disk.** `cache-telemetry.mjs` already holds module-scope state (`legacyCleanupDone`, `lastSweepMs`). Add a `Map<string, { servedTarget, divergentTurnCounter, sticky, firstSeenIso }>` where the map key is the composite `${sessionFilename}|${requestedModel}` (closes Fable r1 B2 — the previous session-only keying was unimplementable for "same target" and would have been polluted by interleaved background utility calls).
   ...
   **Rehydration on map-miss** (closes Fable r1 B1): when the writer encounters a `(sessionFilename, requestedModel)` pair with no map entry, it reads `sessionFilePath(rawSid).json` (precedent: `sessionFilePath` is exported at `cache-telemetry.mjs:55-57` precisely for sibling-reader access) and seeds the entry from any persisted `model_divergence_sticky` + `model_divergence_first_seen` + `served_model` fields. If `model_divergence_sticky: true` is persisted, the new map entry carries `sticky: true` immediately so the next emit re-emits the sticky spread — the indicator survives a proxy restart. Disk read is wrapped in try/catch with a missing-file = fresh-entry fall-through; rehydration is best-effort and never throws into the pipeline.

   LRU-bounded with eviction: the existing `sweepStaleSessions` at `cache-telemetry.mjs:132-156` is extended (or a sibling map-sweep added at the same throttled cadence) to drop map entries whose `sessionFilename` matches an evicted-from-disk session (i.e. the file got swept). `__resetForTests` (`cache-telemetry.mjs:275`) clears the map for unit tests.
...
- **Detector unit tests** (`test/proxy-cache-telemetry-model-divergence.test.mjs`, new file):
  ...
  - **Restart rehydration (Fable r1 B1):** persist a per-session JSON with `model_divergence_sticky: true`; reset the module-scope map (`__resetForTests`); next turn produces a map-miss, the writer rehydrates from disk, and the next emit carries `sticky: true` even though no new divergence happened. Sticky survives restart.
...
- **Statusline rendering** (`test/quota-statusline-divergence.test.sh` or extend the existing statusline test if one is present — confirmed during impl): no divergence → no extra field; recent divergence → red `requested → served`; sticky → black-on-yellow-background treatment; `[1m]` suffix appears on the requested side only when `auto_1m_detected: true`; served-side `[1m]` is never rendered. Tests invoke the script with synthetic per-session JSON files and assert on the rendered string.
...
- [ ] **Map authoritative + rehydrates from on-disk JSON on map-miss.** Persisted `model_divergence_sticky` survives proxy restart. Rehydration disk-read failure is non-fatal (best-effort). (Fable r1 B1.)
- [ ] Per-session state map LRU-bounded and swept by the existing `sweepStaleSessions` cadence (or a sibling map-sweep at the same cadence); `__resetForTests` clears it.
