...
14. **Concurrency**: parallel calls to the writer don't corrupt JSONL. The writer uses single-syscall `fs.promises.appendFile(path, record + "\n")`. POSIX guarantees that an `O_APPEND` open atomically combines offset adjustment with the write, so concurrent appenders cannot lose data to torn offsets. POSIX does NOT guarantee non-interleaved writes to regular files at sub-write granularity (`PIPE_BUF` atomicity, sometimes cited here, applies to pipes/FIFOs only). In practice on Linux with ext4/xfs/btrfs and a single `write(2)` of a buffer well under one page (4096 bytes), record-level interleaving does not occur — empirical kernel behavior, not POSIX. Each event record is well under 4 KB. Test by spawning 50 parallel writes and asserting the resulting file has exactly 50 well-formed JSON lines with no truncation or interleaving; if that test ever fails on a supported platform, escalate to per-record `tmp+rename`, `flock(2)`, or a single-writer queue.
